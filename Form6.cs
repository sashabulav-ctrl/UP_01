...
namespace WinFormsApp_UP_01
{
    public partial class Form6 : Form
    {
        ...
        public void UpdateFormula(string newFormula)
        {
            ...
            this.Text = $"Метод итераций: {newFormula}";
        }
        ...
        private double FindOptimalLambda(string formula, double x0)
        {
            // Оценка производной в окрестности точки
            double h = 1e-5;
            double df = (f(formula, x0 + h) - f(formula, x0 - h)) / (2 * h);

            // Для метода итераций нужно |1 - lambda * f'(x)| < 1
            if (Math.Abs(df) > 1e-10)
            {
                // Оптимальный lambda для быстрой сходимости
                double lambda = 1.0 / df;

                // Проверяем условие сходимости
                double convergenceFactor = Math.Abs(1 - lambda * df);

                if (convergenceFactor >= 1)
                {
                    // Корректируем lambda для обеспечения сходимости
                    lambda = 0.5 / df;
                    convergenceFactor = Math.Abs(1 - lambda * df);
                ...
                // Ограничиваем lambda, чтобы не было слишком больших шагов
                lambda = Math.Max(-1, Math.Min(1, lambda));
                return lambda;
            }
        ...
        private double Phi(string formula, double x, double lambda)
        {
            string cleanFormula = formula.Split('=')[0];
            string phiFormula = "x - " + lambda + "*(" + cleanFormula + ")";
            Expression e = new Expression(phiFormula, new Argument("x", x));
            return e.calculate();
        }

        private double IterationMethod(string formula, double x0, double eps, int maxIter = 1000)
        {
            // Автоматический подбор lambda для обеспечения сходимости
            double lambda = FindOptimalLambda(formula, x0);
            double x_prev = x0;
            double x_current = Phi(formula, x_prev, lambda);
            int iter = 0;

            // Дополнительная корректировка lambda если метод не сходится
            while (Math.Abs(x_current - x_prev) > eps && iter < maxIter)
            {
                if (iter > 100 && Math.Abs(x_current - x_prev) > Math.Abs(x_prev - Phi(formula, x_prev, lambda * 0.5)))
                {
                    // Если сходимость плохая, пробуем уменьшить lambda
                    lambda = lambda * 0.5;
                }

                x_prev = x_current;
                x_current = Phi(formula, x_prev, lambda);
                iter++;
            }
        ...
        private List<double> FindAllRoots(string formula, double a, double b, double eps, int segments = 500)
        {
            ...
            // Сначала проверяем точки на границах и внутри интервала
            ...
            // Ищем интервалы со сменой знака
            for (int i = 0; i < segments; i++)
            {
                double x1 = a + i * step;
                double x2 = x1 + step;
                ...
                if (f1 * f2 < 0)
                {
                    try
                    {
                        // Используем метод итераций с начальным приближением из середины интервала
                        double mid = (x1 + x2) / 2;
                        double root = IterationMethod(formula, mid, eps);
                        double froot = f(formula, root);

                        if (Math.Abs(froot) < eps * 10 && root >= a && root <= b)
                        {
                            if (!IsRootExists(roots, root, eps))
                            {
                                roots.Add(root);
                            }
                        }
                    ...
                    {
                        // Если метод итераций не сошелся, используем просто середину интервала как приближение
                        double mid = (x1 + x2) / 2;
                        if (!IsRootExists(roots, mid, eps))
                        {
                            roots.Add(mid);
                        }
                    }
        ...
        private void UpdateResultsDisplay(List<double> roots)
        {
            ...
            else
            {
                for (int i = 0; i < roots.Count; i++)
                {
                    ...
                    if (i < roots.Count - 1) result += Environment.NewLine;
                }
            }
        ...
        private void btnCalculate_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                // Проверка на нулевую функцию
                bool isZeroFunction = true;
                double stepCheck = (b - a) / 100;
                for (double x = a; x <= b; x += stepCheck)
                {
